...
class ViewMain:
    def __init__(self):
        ...
        self.viewmain.geometry("500x500")
        ...
        self.t_scaler = Label(self.viewmain,text = "Input the scaler:")
        self.e_scaler = Entry(self.viewmain, width=10)

        self.t_minarea = Label(self.viewmain,text = "Input the minimum area of particle:")
        self.e_minarea = Entry(self.viewmain, width=10)

        self.t_scaler.place(x=50,y=10)
        self.e_scaler.place(x=390,y=10, width=60, height=20)
        self.e_scaler.insert(0,self.scaler)

        self.t_minarea.place(x=50,y = 30)
        self.e_minarea.place(x=390,y = 30,width=60, height=20)
        self.e_minarea.insert(0,self.min_area)

        self.b_previous.place(x=50,y=450, width=60, height=30)
        self.b_process.place(x=163,y=450, width=60, height=30)
        self.b_export.place(x=276,y=450, width=60, height=30)
        self.b_next.place(x=390,y=450, width=60, height=30)
    ...
    def processCurrentImage(self,threshold = 25):
        ...
        if len(self.Imgs) == 0:
            ...
            self.scaler = float(self.e_scaler.get())
            ...
            self.img_current.extract(scaler=self.scaler,min_area=self.min_area)
    ...
    def processAllImages(self):
        ...
        if len(self.Imgs) == 0:
            ...
            for i in range(len(self.Imgs)):
                ...
                img.process(5)
                img.extract(scaler=self.scaler,min_area=self.min_area)
    ...
    def DisplayItem(self):
        ...
        self.canvas.place(x=50, y=50, width=400, height=400)
    ...
    def DrawDia(self):
        ...
        if self.img_current == None:
            ...
            self.viewhistdia.title('HistOfDiameter')
            self.viewhistdia.geometry("500x500")

            min_dia = round(self.img_current.range_size[0],2)
            l_min_dia = Label(self.viewhistdia,text = "Input the minimum diameter: {:.2f}".format(min_dia))
            e_min_dia = Entry(self.viewhistdia, width=10)

            max_dia = round(self.img_current.range_size[1],2)
            l_max_dia = Label(self.viewhistdia,text = "Input the maximum diameter: {:.2f}".format(max_dia))
            e_max_dia = Entry(self.viewhistdia, width=10)

            l_interval = Label(self.viewhistdia,text = "Input the interval:")
            ...
            l_min_dia.place(x=50,y=10)
            e_min_dia.place(x=300,y=10, width=60, height=20)
            e_min_dia.insert(0,min_dia)

            l_max_dia.place(x=50,y=30)
            e_max_dia.place(x=300,y=30, width=60, height=20)
            e_max_dia.insert(0,max_dia)

            l_interval.place(x=50,y = 50)
            e_interval.place(x=300,y = 50,width=60, height=20)
            e_interval.insert(0,0.5)

            b_draw.place(x = 390,y = 30,  width=60, height=20)
    ...
    def DrawArea(self):
        ...
        if self.img_current == None:
            ...
            self.viewhistarea.title('HistOfArea')
            self.viewhistarea.geometry("500x500")

            min_area = round(self.img_current.range_size[2],2)
            l_min_area = Label(self.viewhistarea,text = "Input the minimum area:{:.2f}".format(min_area))
            e_min_area = Entry(self.viewhistarea, width=10)

            max_area = round(self.img_current.range_size[3], 2)
            l_max_area = Label(self.viewhistarea,text = "Input the maximum area:{:.2f}".format(max_area))
            e_max_area = Entry(self.viewhistarea, width=10)

            l_interval = Label(self.viewhistarea,text = "Input the interval:")
            ...
            l_min_area.place(x=50,y=10)
            e_min_area.place(x=300,y=10, width=60, height=20)
            e_min_area.insert(0,min_area)

            l_max_area.place(x=50,y=30)
            e_max_area.place(x=300,y=30, width=60, height=20)
    ...
    def DrawHist(self,list,num_max,num_min,interval,Type):
        ...
        plt.title("Distribution histogram").set_fontsize(8)
        ...
        plt.title("Cumulative distribution histogram").set_fontsize(8)
        if Type == "Diameter":
            self.canvas1 = FigureCanvasTkAgg(f, self.viewhistdia)
            self.canvas1.draw()
            self.canvas1.get_tk_widget().place(x=50, y=70, width=400, height=400)

        elif Type == "Area":
            self.canvas2 = FigureCanvasTkAgg(f, self.viewhistarea)
            self.canvas2.draw()
            self.canvas2.get_tk_widget().place(x=50, y=70, width=400, height=400)
